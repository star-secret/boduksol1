...
def convert_return_count(filename): #파일이 주어지면 비율계산
...
def convert_return_portion(count, count_area_linterest):
    portion = (1 - ((count) / count_area_linterest)) * 100  # area_interest에는 주어진 관심영역의 픽셀값이 들어가야함!
    #print(round(portion,2))
    return round(portion,2)  # 비율출력
