...
class AntidroneSprite(pygame.sprite.Sprite):
    ...
    def __init__(self, position,rad):
        pygame.sprite.Sprite.__init__(self)
        self.user_image_normal = pygame.transform.scale(pygame.image.load("emptycircle.png"),(2*rad,2*rad))
        self.user_position = position
...
for i in range(0,len(adx),1):
    antidrones.append(AntidroneSprite((adx[i], ady[i]),400))
...
while True:
    ...
    for event in pygame.event.get():
        ...
        drone.user_yspeed = -go_y#*4/deltat
        drone.user_xspeed = -go_x#*4/deltat
