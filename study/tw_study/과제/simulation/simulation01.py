...
#2 게임창 옵션 설정 - 제목
pygame.display.set_caption('Anti drones 시스템의 배치01')          # 캡션설정 - 창의 이름을 설정

rect = screen.get_rect()    #스크린의 rect정보를 받아옴
simple = SimpleSprite('triangl2.png', (100,100))  #simple이라는 객체를 SimpleSprite클래스를 이용해서 생성할 것
...
rotate = RotateSprite("triangl2.png",(50,50))
rotate_group = pygame.sprite.RenderPlain(rotate)
...
while True:
    ...
    rotate_group.update(deltat)
                      #mask충돌을 이용해서 이미지와 이미지가 겹칠때 충돌이라는 글자와 시작하고 얼마나 흘렀는지 콘솔창에 출력
    if(pygame.sprite.collide_mask(rotate,simple)): 
        print("충돌")
    ...
    rotate_group.clear(screen, background)
