...
class droneSprite(pygame.sprite.Sprite):
    ...
    def __init__(self, image, position):
        ...
        self.user_src_image = pygame.transform.scale(pygame.image.load(image),(10,10))
...
while True:
    ...
    textSurfaceObj = fontObj.render("count is{}".format(drone.user_position), True, (255,0,10))   
    ...
    for event in pygame.event.get():
        ...
        if hasattr(event, 'key'):
            ...
            if event.key == K_RIGHT:
                drone.user_xspeed = down * -2 # 시계 방향이 마이너스인 것에 유의

                
            elif event.key == K_LEFT:
                drone.user_xspeed = down * 2
            elif event.key == K_UP:
                drone.user_yspeed = down * 2
